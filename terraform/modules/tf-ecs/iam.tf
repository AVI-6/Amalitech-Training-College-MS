...
####Lambda execution role for ECS notifications
resource "aws_iam_role" "ecs_notify_lambda_role" {
  name = "${var.name}-ecs-notify-lambda-role" 
  assume_role_policy = jsonencode({
    Version = "2012-10-17"
    Statement = [{
      Effect    = "Allow"
...
resource "aws_iam_role_policy" "ecs_notify_lambda_policy" {
  name = "${var.name}-ecs-notify-lambda-policy"
  role = aws_iam_role.ecs_notify_lambda_role.id
